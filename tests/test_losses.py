    ...
    **Tests de las 3 funciones de lossses.py**\n
    ...
    def test_effective_number_weights(self):
        ...
        **salida esperada:**\n
        ...
        labels = np.array([3,2,5,5,5,5,5,5,5,1,1,1,1,1,1,1,1])
        weights = effective_number_weights(labels, 5)

        labels1 = np.array([3,2,5]) #da 1 si no hay ejemplos
        weights1 = effective_number_weights(labels1, 5)

        print("\n|| test_effective_number_weights ||")
        print("weights:")
        print(weights, "\n wc_mean:", weights.mean())
        print("\nweights1:")
        print(weights1, "\n w1c_mean:", weights1.mean())

        self.assertEqual(weights.mean().item(), 1.)
        self.assertEqual(weights1.mean().item(), 1.)
