...
adicionales de la corrida final completa. Este último entrega argmax y
...
`GDS_R2` sin entrenar otro modelo. Las tablas y figuras de la corrida final
están en [docs/resultados](docs/resultados/MANIFIESTO.md).
...
índices `0, …, K−1`. Para `GDS_R3`, original `1/3` equivale a índice `0/1`.
Los CSV y PNG de confusión rotulan las clases originales en ambos ejes.
...
| [`docs/resultados/`](docs/resultados/MANIFIESTO.md) | Tablas y confusiones OOF finales, versionadas. |
...
Con el código actual pasaron 30 pruebas. El benchmark de un ajuste
de 20 épocas en un fold de `GDS_R2` dio mediana de 0,223 s en CPU y 0,428 s
en GPU para tres repeticiones; se eligió CPU con un hilo OpenMP. Comando de
reproducción desde la raíz del repositorio, con una carpeta de salida nueva:
...
En el servidor se usó `~/.envs/lab_pytorch/bin/python`; `estado.json`
guarda los argumentos efectivos. La ejecución terminó el **23/09/2026**.
El manifiesto de la corrida registra comando, entorno, SHA del SAV y hashes
del código efectivamente ejecutado. Entorno del servidor: Python 3.10.21,
PyTorch 2.14.0, NumPy 2.2.6, pandas 2.3.3, scikit-learn 1.7.2 y
pyreadstat 1.3.6. Procedencia y verificación constan en el
[manifiesto de resultados](docs/resultados/MANIFIESTO.md).

La ejecución produce `estado.json`, `resultados.csv`,
`configuraciones_folds.csv`, `predicciones_oof.csv`, `verificacion_oof.csv`
y reportes bajo `<método>/<objetivo>/fold_XX/`. En esta corrida,
...
`base_*` de las tablas son parámetros CLI de referencia, no HP elegidos. Las
cifras del [CSV versionado](docs/resultados/resultados.csv) y las confusiones
proceden de la corrida validada. Los artefactos por fold y las predicciones
individuales se reconstruyen con el comando y quedan en `results/`.
...
| CORN | `K−1` logits condicionales; suma la BCE de los pares elegibles y divide por su número total, según las ecuaciones 5–6 del artículo. | Producto acumulado de condicionales y conteo de umbrales. |
...
promedio aplicado al train puede ser distinto de uno. Se eligió β=0,9
...
El grid fijado antes de la corrida, en orden `(hidden_dim, dropout, learning_rate,
...
configuración; no hubo early stopping ni cambios de grid o épocas motivados
por el test externo.
...
La corrida terminó sin fallos: 42 filas, 189 configuraciones por fold, nueve
...
comparten probabilidades y folds. Las filas «CORN» usan la pérdida descrita
en la sección 5. Véanse
[cobertura](docs/resultados/verificacion_oof.csv),
[configuraciones](docs/resultados/configuraciones_folds.csv) y
[manifiesto](docs/resultados/MANIFIESTO.md).

## 7. Resultados definitivos por objetivo

Cada celda muestra **media ± desviación** entre folds externos, redondeada a
tres decimales. El [CSV definitivo](docs/resultados/resultados.csv)
...
| CORN | 0.559 ± 0.009 | 0.286 ± 0.004 | 0.294 ± 0.013 | 0.286 ± 0.004 | 0.279 ± 0.001 | 0.487 ± 0.003 | 0.676 ± 0.007 | 0.958 ± 0.006 | 0.042 ± 0.006 |
...
con mediana](docs/resultados/matrices/softmax_matched_median/GDS.png).
...
| CORN | 0.895 ± 0.024 | 0.697 ± 0.053 | 0.761 ± 0.059 | 0.697 ± 0.053 | 0.709 ± 0.043 | 0.107 ± 0.024 | 0.672 ± 0.062 | 0.998 ± 0.004 | 0.002 ± 0.004 |
...
La mediana equiparada logró F1 macro 0,731 y CORN 0,709, frente a 0,505 de
...
| CORN | 0.726 ± 0.034 | 0.645 ± 0.041 | 0.708 ± 0.041 | 0.645 ± 0.041 | 0.668 ± 0.041 | 0.290 ± 0.037 | 0.687 ± 0.046 | 0.985 ± 0.005 | 0.015 ± 0.005 |
...
La mediana equiparada dio F1 macro 0,677 y MAE 0,283; CORN 0,668 y 0,290;
...
| CORN | 0.832 ± 0.008 | 0.527 ± 0.036 | 0.533 ± 0.044 | 0.527 ± 0.036 | 0.521 ± 0.027 | 0.168 ± 0.008 | 0.307 ± 0.044 | 1.000 ± 0.000 | 0.000 ± 0.000 |
...
![Confusión OOF de CORAL sin pesos en GDS_R4](docs/resultados/matrices/coral/GDS_R4.png)
...
| CORN | 0.772 ± 0.009 | 0.505 ± 0.012 | 0.514 ± 0.019 | 0.505 ± 0.012 | 0.500 ± 0.012 | 0.228 ± 0.009 | 0.420 ± 0.020 | 1.000 ± 0.000 | 0.000 ± 0.000 |
...
![Confusión OOF de CORAL sin pesos en GDS_R5](docs/resultados/matrices/coral/GDS_R5.png)
...
un ganador único sin definir qué error importa. Los controles también
variaron por objetivo: CORN obtuvo MAE 0,487 en `GDS`, pero en
`GDS_R5` quedó por debajo de Softmax HP en F1 (0,500 frente a 0,510) y MAE
(0,228 frente a 0,222). En `GDS_R3` el baseline siguió competitivo.
...
la sección 5 y las tablas y configuraciones versionadas. Este README sirve
de base para el informe técnico.
